...
import fuzs.nightconfigfixes.core.CommonAbstractions;
...
public class JsonConfigFileUtil {
    ...
    public static boolean copyToFile(File jsonFile) {
        ...
        try (InputStream input = JsonConfigFileUtil.class.getResourceAsStream("/" + jsonFile.getName()); FileOutputStream output = new FileOutputStream(jsonFile)) {
            ...
            NightConfigFixes.LOGGER.error("Failed to copy {} in config directory: {}", jsonFile.getName(), e);
        }
        return false;
    }
    ...
    public static boolean saveToFile(File jsonFile, JsonElement jsonElement) {
        ...
        try (FileWriter writer = new FileWriter(jsonFile)) {
            ...
            NightConfigFixes.LOGGER.error("Failed to create {} in config directory: {}", jsonFile.getName(), e);
        }
        return false;
    }
    ...
    private static void loadFromFile(File file, Consumer<FileReader> deserializer) {
        try (FileReader reader = new FileReader(file)) {
            ...
            NightConfigFixes.LOGGER.error("Failed to read {} in config directory: {}", file.getName(), e);
        }
    }
            ...
            for (File file : allFilesAndDirs) {
                if (file.isDirectory()) {
                    ...
                    try {
                        ...
                        NightConfigFixes.LOGGER.error("Failed to locate files in {} directory: {}", directory.getName(), e);
                    }
                }
            }
        }
